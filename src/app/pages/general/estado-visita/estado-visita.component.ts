...
@Component({
  selector: 'estado-visita-table',
  templateUrl: './estado-visita.component.html',
  styleUrls: ['./estado-visita.component.scss'],
})
export class EstadoVisitaComponent {
  idForm: string = '';
  nombreEstado: string = '';
  mantenedor: string = "Estado Visita Técnica";
  ...
  constructor(private estadoVisitaService : EstadoVisitaService,
  ...
  private loadInitialData() {
    this.estadoVisitaService.sendGetRequest().subscribe((data: any[]) => {
      this.source = new ServerDataSource(this.httpClient,
        {
          endPoint: ServiceConstants.GET_ESTADO_VISITA_PATH, //full-url-for-endpoint without any query strings
  ...
  saveButton(){
    if(this.idForm === ''){
      this.estadoVisitaService.save(this.nombreEstado).subscribe((data: any[]) => {
        this.estadoVisitaService.sendGetRequest().subscribe((data: any[]) => {
          this.source.load(data[this.responseListName]);
        })
      },this.manejarErrorSave());
    } else {
      this.estadoVisitaService.update(this.idForm , this.nombreEstado).subscribe((data: any[]) => {
        this.estadoVisitaService.sendGetRequest().subscribe((data: any[]) => {
          this.source.load(data[this.responseListName]);
        })
      },this.manejarErrorSave());

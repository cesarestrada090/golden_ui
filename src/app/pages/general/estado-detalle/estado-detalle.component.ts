...
export class EstadoDetalleComponent {
  idForm: string = '';
  nombreEstado: string = '';
  mantenedor: string = "Estado Detalle";
  ...
  private loadInitialData() {
    this.estadoDetalleService.sendGetRequest().subscribe((data: any[]) => {
      this.source = new ServerDataSource(this.httpClient,
        {
          endPoint: ServiceConstants.GET_ESTADO_DETALLE_PATH, //full-url-for-endpoint without any query strings
  ...
  saveButton(){
    if(this.idForm === ''){
      this.estadoDetalleService.save(this.nombreEstado).subscribe((data: any[]) => {
        this.estadoDetalleService.sendGetRequest().subscribe((data: any[]) => {
          this.source.load(data[this.responseListName]);
        })
      },this.manejarErrorSave());
    } else {
      this.estadoDetalleService.update(this.idForm , this.nombreEstado).subscribe((data: any[]) => {
        this.estadoDetalleService.sendGetRequest().subscribe((data: any[]) => {
          this.source.load(data[this.responseListName]);
        })
      },this.manejarErrorSave());
